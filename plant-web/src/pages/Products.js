...
class Product extends Component {
    ...
    componentDidUpdate(PrevProps) {
        if(!equal(this.props.match.params.type, PrevProps.match.params.type) || !equal(this.props.match.params.page, PrevProps.match.params.page)) {
            this.getDB();
            this.getPage();
        }
    }
                            ...
                            <div className="pagination_wrap">
                                {
                                    numPage.map(num => (
                                        (num != page) ?
                                        <Link to={"/product/type="+type+"/page="+num} onClick={window.scrollTo(0, 0)}>{num}</Link> :
                                        <Link to={"/product/type="+type+"/page="+num} style={{color: "#fff", background: "#000"}} onClick={window.scrollTo(0, 0)}>{num}</Link>
